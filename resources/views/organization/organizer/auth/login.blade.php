@extends('organization.organizer.disconnected-base')
    ...
    <div x-data="{ showLoginForm: true, showPasswordResetForm: false }"
        ...
        <div x-show="showLoginForm" x-transition
            ...
            <form action="" method="POST">
                ...
                <div class="flex items-center justify-between mb-6">
                    <a href="{{route('organization.organizer.auth.disconnected.signupView')}}" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 cursor-pointer">
                        Pas encore inscrit ?
                    </a>
                </div>
        ...
        <div x-show="showPasswordResetForm" x-transition>
            <livewire:auth.password-reset-while-disconnected-form :guard="'organizer'" />
        </div>
    </div>
@endsection

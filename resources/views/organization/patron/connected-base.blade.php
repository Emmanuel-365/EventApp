...
<nav class="bg-white dark:bg-gray-800 shadow-sm transition-colors duration-300 ease-in-out">
    <div class="container mx-auto px-4 py-3 flex justify-between items-center">
        <a href="{{route('patron.patronPanel')}}" class="text-2xl font-bold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">
            Patron
        </a>
        ...
        <div class="hidden md:flex items-center space-x-6">
            <a href="{{route('patron.patronPanel')}}" class="text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200">Employés</a>

            <form action="{{route('patron.logout')}}" method="POST">
                @csrf
                @method('DELETE')
    ...
    <div x-show="sidebarOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-y-4" x-transition:enter-end="opacity-100 translate-y-0"
        ...
        <a href="{{route('patron.patronPanel')}}" class="block text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 py-1">Employés</a>
        <form action="{{route('patron.logout')}}" method="POST" class="pt-2">
            @csrf
            @method('DELETE')
...
<footer class="bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 p-4 text-center shadow-inner mt-auto transition-colors duration-300 ease-in-out">
    <div class="container mx-auto">
        <p class="text-sm md:text-base">&copy; {{ date('Y') }} Patron Panel. Tous droits réservés.</p>
    </div>
</footer>
</body>
</html>

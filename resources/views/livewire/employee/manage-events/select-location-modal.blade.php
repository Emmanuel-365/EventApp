<div x-data="{ show: @entangle('show').live, map: null, marker: null, autocomplete: null, geocoder: null }"
     x-init="
        ...
        const reverseGeocode = (latLng) => {
            ...
            geocoder.geocode({ 'location': latLng }, (results, status) => {
                if (status === 'OK' && results[0]) {
                    ...
                    @this.set('address', 'Adresse non trouvée'); // Fallback si l'adresse n'est pas trouvée
                }
            });
        };
                ...
                setTimeout(() => {
                    if (map) {
                        ...
                        if (@this.latitude && @this.longitude) {
                            const currentLatLng = new google.maps.LatLng(@this.latitude, @this.longitude);
        ...
        <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    {{-- Champ de recherche pour l'autocomplétion Google Places --}}
                    <div class="mb-4">
                    ...
                    {{-- Affichage des coordonnées et de l'adresse sélectionnées (pour débogage ou feedback) --}}
                    @if ($latitude && $longitude)
                        <div class="mt-4 text-sm text-gray-700 dark:text-gray-300">
                            <strong>Lat:</strong> {{ number_format($latitude, 5) }},
                            <strong>Lng:</strong> {{ number_format($longitude, 5) }}
                            <br>
                            <strong>Adresse:</strong> {{ $address ?? 'N/A' }}
                        </div>
                    @else
        ...
        <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 rounded-b-lg">
            <button type="button"
                    @click="() => {
                        ...
                        @this.call('selectLocation', latitude, longitude, address);
                    }"
                    ...
                    x-bind:disabled="!latitude || !longitude || !address"> {{-- Désactivé si pas de sélection --}}

<div>
    {{-- Message de succès global (pour les actions de création et de détail) --}}
    @if ($globalSuccessMessage)
        <div x-data="{ show: true }" x-show="show" x-transition:leave.opacity.duration.1000ms
             x-init="setTimeout(() => { show = false; @this.set('globalSuccessMessage', null); }, 5000);"
             class="max-w-7xl mx-auto px-4 py-3 rounded relative mb-6 bg-green-100 border border-green-400 text-green-700"
             role="alert">
            <strong class="font-bold">Succès !</strong>
            <span class="block sm:inline">{{ $globalSuccessMessage }}</span>
            <span class="absolute top-0 bottom-0 right-0 px-4 py-3 cursor-pointer" @click="show = false; @this.set('globalSuccessMessage', null);">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                @if($organization->validation_status === 'pending') bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100
                                @elseif($organization->validation_status === 'accepted') bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                @if($organization->activation_status === 'enabled') bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100
                                @else bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100 @endif">
                                {{ ucfirst($organization->activation_status) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button wire:click="$dispatch('openOrganizerOrganizationDetailsModal', { organizationId: '{{ $organization->id }}' })"

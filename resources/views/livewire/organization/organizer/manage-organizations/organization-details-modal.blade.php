<div x-data="{ showModal: @entangle('show').live }"
    ...
    <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg p-6 sm:p-8 w-full max-w-2xl relative" @click.stop>
        ...
        @if ($organization)
            ...
            {{-- Message d'erreur interne à la modale --}}
            @if ($errorMessage)
            ...
            <div class="space-y-4 text-gray-700 dark:text-gray-300 mb-6">
                <p><strong>NIU:</strong> {{ $organization->NIU }}</p>
                <p><strong>Type:</strong> {{ $organization->type }}</p>
                <p><strong>Date de création:</strong> {{ \Carbon\Carbon::parse($organization->date_creation)->format('d/m/Y') }}</p>
                <p><strong>Sous-domaine:</strong> {{ $organization->domains->first()->domain ?? 'N/A' }}</p>
                <p>
                    ...
                    @if ($organization->validation_status === 'rejected' && $organization->rejected_reason)
                        <span class="text-red-500 text-sm ml-2">({{ $organization->rejected_reason }})</span>
                    @endif
                </p>
                <p>
                    ...
                    @if ($organization->activation_status === 'disabled' && $organization->disabled_reason)
                        <span class="text-red-500 text-sm ml-2">({{ $organization->disabled_reason }})</span>
                    @endif
                </p>
            </div>
            ...
            <div class="mt-8 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                @elseif ($organization->activation_status === 'disabled')
                    {{-- Vérifier si l'organisateur peut réactiver cette organisation --}}
                    @php

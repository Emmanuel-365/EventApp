<div>
    ...
    <div class="mb-6 flex flex-col md:flex-row justify-between items-stretch md:items-center space-y-4 md:space-y-0 md:space-x-4">
        ...
        <!--[if BLOCK]><![endif]--><?php if($user && $user->can('create-events')): ?>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden transition-colors duration-300 ease-in-out">
                ...
                <!--[if BLOCK]><![endif]--><?php $__empty_1 = true; $__currentLoopData = $events; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-150 ease-in-out">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                            <?php echo e(\Carbon\Carbon::parse($event->date)->format('d/m/Y')); ?> à <?php echo e(\Carbon\Carbon::parse($event->time)->format('H:i')); ?>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                <span class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full
                                    ...
                                    <?php elseif($event->status === 'not_published'): ?> bg-gray-200 text-gray-700 dark:bg-gray-600 dark:text-gray-200
                                    <?php elseif($event->status === 'cancelled'): ?> bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100
                                    <?php elseif($event->status === 'pending_cancellation' || $event->status === 'pending_deletion'): ?> bg-yellow-100 text-yellow-800 dark:bg-yellow-700 dark:text-yellow-100
                                    <?php endif; ?>
                                ">
                                    <?php echo e(ucfirst(str_replace('_', ' ', $event->status))); ?>

                                </span>
                            <!--[if BLOCK]><![endif]--><?php if($event->deleted_at): ?>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center space-x-2 justify-center">
                                <!--[if BLOCK]><![endif]--><?php if($user && $user->can('see-events')): ?>
                                    <button wire:click="viewEventDetails('<?php echo e($event->id); ?>')"
                                ...
                                <!--[if BLOCK]><![endif]--><?php if($user && $user->can('update-events') && $event->status === 'published' && !$event->deleted_at): ?>
                                    <button wire:click="confirmCancelEvent('<?php echo e($event->id); ?>')"
                                ...
                                <!--[if BLOCK]><![endif]--><?php if($user && $user->can('restore-events') && ($event->status === 'cancelled' || $event->deleted_at)): ?>
                                    <button wire:click="restoreEvent('<?php echo e($event->id); ?>')"
                                ...
                                <!--[if BLOCK]><![endif]--><?php if($user && $user->can('delete-events') && !$event->deleted_at): ?>
                                    <button wire:click="confirmDeleteEvent('<?php echo e($event->id); ?>')"
                                ...
                                <!--[if BLOCK]><![endif]--><?php if($user && $user->can('refund-tickets') && ($event->status === 'pending_cancellation' || $event->status === 'pending_deletion')): ?>
                                    <button wire:click="initiateRefunds('<?php echo e($event->id); ?>')"
                                ...
                                <!--[if BLOCK]><![endif]--><?php if($user && $user->can('see-tickets')): ?>
                                    <a href="<?php echo e(route('employee.manageTickets', ['event' => $event])); ?>"
        ...
        <div class="p-4 border-t border-gray-200 dark:border-gray-700">
            <?php echo e($events->links('pagination::tailwind')); ?>

        </div>
    </div>
</div>
<?php /**PATH C:\Users\MARCAU\PhpstormProjects\EventApp\resources\views/livewire/employee/manage-events/list-events.blade.php ENDPATH**/ ?>